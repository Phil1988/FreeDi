...
# 
# Modified by Phil1988 for the use of FreeDi at the Plus4 to make it act as a filament detection sensor only
# Changes marked as #!
...
class HallFilamentWidthSensor:
    def __init__(self, config):
        ...
        # Инициализация ADC пинов с автоопределением формата callback
        self._init_adc_pins()
    ...
    def _init_adc_pins(self):
        """Инициализация ADC пинов с определением версии API"""
        self.ppins = self.printer.lookup_object('pins')
        
        # Настраиваем первый ADC пин
        self.mcu_adc = self.ppins.setup_pin('adc', self.pin1)
        self.mcu_adc.setup_adc_sample(ADC_SAMPLE_TIME, ADC_SAMPLE_COUNT)
        
        # Настраиваем второй ADC пин
        self.mcu_adc2 = self.ppins.setup_pin('adc', self.pin2)
        self.mcu_adc2.setup_adc_sample(ADC_SAMPLE_TIME, ADC_SAMPLE_COUNT)
        
        # Пробуем разные варианты setup_adc_callback
        self._setup_adc_callbacks()
    
    def _setup_adc_callbacks(self):
        """Пробует разные форматы setup_adc_callback"""
        
        # Вариант 1: Новый API с 3 аргументами (time, self, callback)
        try:
            ...
            # Если сработало, используем этот вариант
            return
        except TypeError:
            pass
        
        # Вариант 2: Средний API с 2 аргументами (time, callback)
        try:
        ...
        # Вариант 3: Старый API с 1 аргументом (callback)
        try:
            ...
            # Если ничего не работает, выводим ошибку
            raise self.printer.config_error(
                "Не удалось настроить ADC callback. "
                "Проверьте версию Klipper. Ошибка: %s" % str(e))
    ...
    def _calculate_diameter(self):
        """Расчет диаметра filament"""
        if hasattr(self, 'lastFilamentWidthReading') and hasattr(self, 'lastFilamentWidthReading2'):
    ...
    def extrude_factor_update_event(self, eventtime):
        ...
        self.runout_helper.note_filament_present(eventtime,
            self.runout_dia_min <= self.diameter)
